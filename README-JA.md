...
**複数のクライアント専用大規模言語モデルAPI（Kiro ...）を模擬リクエストし、ローカルのOpenAI互換インターフェースに統一的にラッピングする強力なプロキシ。**
...
> - **2025.11.16** - Ollamaプロトコルサポートの追加、統一インターフェースでサポート対象モデル（Claude、OpenAIなど）にアクセス
...
### 🎯 統一アクセス、ワンストップ管理
*   **マルチモデル統一インターフェース**：標準OpenAI互換プロトコルを通じて、一度の設定でClaude、Kimi K2、MiniMax M2などの主流大規模モデルにアクセス
...
### 🚀 制限を突破、効率を向上
*   **公式制限の回避**：OAuth認証メカニズムを利用して、KiroなどのOAuth系プロバイダーに接続
*   **無料高度モデル**：Kiro APIモードでClaude Opus 4.5を無料使用し、使用コストを削減
...
1. **認証の生成**：**「プロバイダープール」** ページまたは **「設定管理」** ページで、対応するプロバイダー（Kiroなど）の右上にある **「認証生成」** ボタンをクリックします。
...
**解決策**：
- **ネットワーク接続を確認**：AWSなどのサービスに正常にアクセスできることを確認
